    ...
    fields: [
      {
        ...
        help: 'Initial State Variable: Total money the startup begins with at time t=0 (e.g., seed investment).',
        min: 0, max: 50000000, step: 10000, unit: '₹', color: 'var(--nb-yellow)',
      },
      {
        key: 'monthlyRevenueBase',
        label: 'Monthly Income',
        help: 'Deterministic Input: The baseline money coming in each month before adding random noise.',
        min: 0, max: 10000000, step: 5000, unit: '₹', color: 'var(--nb-yellow)',
      },
      {
        key: 'revenueGrowthRate',
        label: 'Growth Rate',
        help: 'Time-Series Growth: How fast the baseline income grows (compounds) over each discrete time step (month).',
        min: -20, max: 100, step: 0.1, unit: '%', color: 'var(--nb-green)',
      },
      {
        key: 'revenueRandomness',
        label: 'Income Randomness',
        help: 'Stochastic Variance (σ): How unpredictable your income is (Normal/Gaussian Distribution). High = variable sales.',
    ...
    fields: [
      {
        ...
        help: 'Deterministic Cost: Fixed expenses subtracted at every time step (rent, salaries, etc).',
        min: 0, max: 10000000, step: 5000, unit: '₹', color: 'var(--nb-red)',
      },
      {
        key: 'costSpikeProbability',
        label: 'Surprise Cost Chance',
        help: 'Discrete Event Probability: Chance (Bernoulli trial) of a sudden unexpected expense occurring in any given month.',
    ...
    fields: [
      {
        key: 'costSpikeMin',
        label: 'Surprise Min',
        help: 'Uniform Distribution Min: Smallest cost amount when a surprise event is triggered.',
        min: 0, max: 5000000, step: 5000, unit: '₹', color: 'var(--nb-red)',
      },
      {
        key: 'costSpikeMax',
        label: 'Surprise Max',
        help: 'Uniform Distribution Max: Largest possible cost when a surprise event is triggered.',
    ...
    fields: [
      {
        ...
        help: 'Discrete Event Probability: Chance of securing investor funding in any month. 0% = bootstrapped system.',
        min: 0, max: 100, step: 1, unit: '%/mo', color: 'var(--nb-blue)',
      },
      {
        key: 'fundingMin',
        label: 'Funding Min',
        help: 'Uniform Distribution Min: Smallest possible funding amount if the funding event occurs.',
        min: 0, max: 100000000, step: 50000, unit: '₹', color: 'var(--nb-blue)',
      },
      {
        key: 'fundingMax',
        label: 'Funding Max',
        help: 'Uniform Distribution Max: Largest possible funding amount if the funding event occurs.',
    ...
    fields: [
      {
        ...
        help: 'Macro-Economic Noise: Base variance applied to all monetary variables simulating unpredictability of the market.',
        min: 0, max: 100, step: 1, unit: '%', color: 'var(--nb-purple)',
      },
      {
        key: 'simulationDuration',
        label: 'Time Period',
        help: 'Time Horizon (t): Number of discrete steps (months) to run the simulation loop. 36 months = 3 years.',
        min: 1, max: 240, step: 1, unit: 'months', color: 'var(--nb-purple)',
      },
      {
        key: 'numSimulations',
        label: 'Number of Runs',
        help: 'Sample Size (N): Number of Monte Carlo iterations. Higher N = closer to true expected probability (Law of Large Numbers).',
